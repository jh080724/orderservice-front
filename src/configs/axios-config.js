...
/*
 ...
 요청을 수행하거나, 응답에 대한 결과 처리를 수행할 수 있습니다.
*/

// Request 용 인터셉터 설정
// 인터셉터의 use함수는 매개값을 2개 받습니다. 둘 다 콜백 함수로 전달합니다.
// 1번째 콜백에는 정상 동작 로직을 작성
// 2번재 콜백에는 과정 중 에러가 발생할 경우 실행할 로직을 작성
axiosInstance.interceptors.request.use(
  (config) => {
    // 요청보내기 전에 일괄 처리해야할 내용을 콜백함수로 전달
...
axiosInstance.interceptors.response.use(
  (response) => response, // 응답에 문제 없으면 그대로 응답객체 리턴,
  ...
  async (error) => {
    ...
    // 원본 요청의 정보를 기억해 놓자 -> 새 토큰 발급 받아서 다시 보내여 되나까.
    ...
    if (error.response.status === 401 && !originalRequest._retry) {
      ...
      try {
        ...
        const token = res.data.result.token; // axios는 json() 안씀. 알아서 자바스크립트 객체를 줌.;
        localStorage.setItem('ACCESS_TOKEN', token); // 동일한 이름으로 토큰 담기(이름 동일하면 덮어씀.);
        ...
        // axiosInstance를 사용하여 다시 한번 원본 요청을 보내고, 응답값은 원래 호출한 곳으로 리턴.
        return axiosInstance(originalRequest);
      } catch (e) {
        console.log(e);
        // 서버에서 Refresh_Token도 만료된 상황. -> 로그아웃된 것 처럼 보여줘야함.
