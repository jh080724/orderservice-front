...
const ProductList = ({ pageTitle }) => {
  ...
  const handleAddToCart = () => {
    ...
    const selectedProduct = Object.keys(selected); // 키값만 뽑아서 배열에 담기.

    // Key 값만 뽑아서 Selected에 들어있는 상품들 중에 false인 거 빼고 true인것만 담겠다.
    ...
    console.log('최종선택: ', finalSelected);
  ...
  const handleCheckboxChange = (productId, checked) => {
    // 사용자가 특정상품을 선택했는지에 대한 상태를 관리{상품아이디, 체크여부}
    setSelected((prevSelected) => ({
      ...prevSelected, // 기존에 담겨있는 것은 그대로 두고...
      [productId]: checked,
    }));
  };
