...
const ProductList = ({ pageTitle }) => {
  ...
  // 현재 로딩중인가 ? -> 백앤드로부터 상품 목록 요청을 보내서 데이터를 받아오는 중인가 ?
  ...
  useEffect(() => {
    ...
    // 쓰로틀링: 짧은시간동안 연속해서 발생한 이벤트들을 일정시간으로 그룹화하여
    // 마지막 이벤트 핸들러만 호출하게 하는 기능.
    // 디바운싱: 짧은시간동안 연속해서 발생한 이벤트를 호출하지 않다가 마지막 이벤트부터
    // 일정시간 잏루에 란번만 호출하게 하는 기능
    const throttledScroll = throttle(scrollPagination, 1000);
    window.addEventListener('scroll', throttledScroll);
    // 클린업 함수: 다른 컴포넌트가 랜더링 될때 이벤트 해제.
    return () => window.removeEventListener('scroll', throttledScroll);
  }, []);

  // useEffect는 하나의 컴포넌트에서 여러 개 선언이 가능함.
  // 스크롤 이벤트에서 다음 페이지 번호를 준비했고,
  // 상태가 바뀌면 그 때 백앤드로 요청을 보낼 수 있게 로직을 나눔.
  useEffect(() => {
  ...
  const loadProduct = async () => {
    // 아직 로딩중이라면 or 이미 마지막 페이지라면 더이상 진행하지 말아라.
    ...
    try {
      ...
      setIsLoading(false); // 백앤드와 연동이 끝났다.
    }
  };

  const scrollPagination = () => {
    // 브라우저 창의 높이 + 현재 페이지에서 스크롤된 픽셀 값
    //  >= (스크롤이 필요 없는 ) 페이지 전체 높이에서 100px이내에 도달했는가?
    const isBottom =
      // rm: 브라우저 창의 높이 + 현재 스크롤된 위치 >= 페이지 전체 높이에서 200px 이내에 도달했는가?
      // rm: window.innerHeight + window.scrollY >= document.body.offsetHeight - 200;
      window.innerHeight + document.documentElement.scrollTop >=
      document.documentElement.scrollHeight - 100;
    if (isBottom && !isLastPage && !isLoading) {
      // loadProduct();
      // 스크롤이 특정 구간에 도달하면 바로 요청보내는게 아니라 다음 페이지 번호를 준비하겠다.
